...
########################### Stuff for altering the network
    ...
    def addConnectingWeight(self, weight_parchild_tuple, val=None, std=1.0):
        ...
        assert (child_atom_index not in self.no_input_allowed_indices) , 'Cant make an input or bias atom a child! weight_tuple {}, \n{}'.format(weight_parchild_tuple, self.printNetwork())
    ...
    def getNetworkFunction(self):
        ...
        if len(self.atom_weight_tensors) > 0:
            ...
            self.NN_fn = lambda x: torch.zeros(self.N_outputs, dtype=torch.float32)
    ...
    def is_legal_weight_add(self, par_index, child_index):

        if (par_index >= self.N_atoms) or (child_index >= self.N_atoms):
            return(False)

        if par_index == child_index:
            return(False)

        par_atom = self.atom_list[par_index]
        child_atom = self.atom_list[child_index]

        # check that they're not both input atoms or output atoms
        if par_atom.is_output_atom:
            return(False)
        if child_index in self.no_input_allowed_indices:
            return(False)

        if self.propagate_order.index(par_index) > self.propagate_order.index(child_index):
            return(False)

        # Get random output, input indices for the atoms
        par_atom_output_index = 0
        child_atom_input_index = 0
        ...
        weight_parchild_tuple = (par_index, par_atom_output_index, child_index, child_atom_input_index)
    ...
    def is_legal_atom_add(self, par_index, child_index):
    ...
    def is_legal_weight_remove(self, par_index, child_index):
...
###################################### Stuff for propagating and actually using the network
    ...
    def backProp(self, output, target):
        ...
        if self.atom_weight_tensors:
            ...
            if output.requires_grad:
                ...
                '''alpha = 10**-2
                with torch.no_grad():
                    for i,w in enumerate(self.atom_weight_tensors):
    ...
    def createOptim(self):
        self.optimizer = optim.Adam(self.atom_weight_tensors, lr=0.01)
...
################################## Other stuff
    ...
    def plotNetwork(self, **kwargs):
        ...
        for atom_ind in self.non_node_atom_indices:
            ...
            x_incr_in = span/max(1, N_in - 1)
            x_incr_out = span/max(1, N_out - 1)
    ...
    def saveNetworkAsAtom(self, **kwargs):
        ...
        try:
            ...
            for output_index in range(self.N_outputs):
                ...
                for atom in self.atom_list:
                    for i,(w_str,w_symbol) in enumerate(atom.output_weights_symbols.values()):
                        NN_fn_atom_format[output_index] = NN_fn_atom_format[output_index].subs(w_str, self.atom_weight_tensors[i].item())
    ...
    def dummy_legal_fn(self, a, b, **kwargs):
        # do nothing
        pass
    ...
    def dummy_fn(self, weight_parchild_tuple, **kwargs):
        # do nothing
        pass
...
# I think this will fix minor things when first adding nodes and

        ...
        for i in range(N_gen):
            ...
            for j, individ in enumerate(self.population):
                ...
                for run in range(N_trials_per_agent):
                    ret = individ.runEpisode(N_episode_steps, **kwargs)
                    mean_episode_score += ret['r_avg']
                    train_curves.append(ret['train_curve'])
            ...
            if i%max(1, int(N_gen/20))==0:
                ...
                best_individ_ind = self.sortByFitnessFunction(mean_Rs)[0][0]
                self.population[best_individ_ind].plotNetwork(show_plot=False, save_plot=True, fname=fname, atom_legend=False)
                plt.clf()
                plt.plot(train_curves[best_individ_ind])
                plt.savefig(fst.combineDirAndFile(self.plot_dir,'train_curve_gen{}.png'.format(i)))
            ...
            for run in range(N_runs_each_champion):
                champion_scores.append((self.population[0].runEpisode(N_episode_steps)['r_avg']))

            champion_FF_mean_std.append([np.mean(champion_scores), np.std(champion_scores)])

            if reward_stop_goal is not None:
                if i>=stop_gen:
                    print('reached generation', stop_gen, ': stopping.')
                    break

                if (best_FF >= reward_stop_goal) and (stop_gen == N_gen):
                    print('reward_stop_goal ({}) reached: {}, in gen {}'.format(reward_stop_goal, best_FF, i))
